# PyClips Website

Marketing site, accounts, Google sign-in, and Razorpay Premium for [pyclips.in](https://pyclips.in).

The Windows desktop clip editor is **not** in this repository.

[![Website](https://img.shields.io/badge/website-pyclips.in-7C5CFF)](https://pyclips.in)
[![License](https://img.shields.io/badge/License-Proprietary-red.svg)](LICENSE)

## License

PyClips is **proprietary software**. All rights reserved — see [LICENSE](LICENSE).

Third-party libraries are listed in [NOTICE](NOTICE) and [docs/THIRD_PARTY_LICENSES.md](docs/THIRD_PARTY_LICENSES.md). Provenance: [docs/PROVENANCE.md](docs/PROVENANCE.md).

## About PyClips

PyClips turns long videos into captioned short-form clips. Paste a link or drop a file, pick a look, and export vertical, square, or landscape MP4s ready for Reels, Shorts, and TikTok.

The desktop app follows Import → Clip → Soundtrack → Captions → Style → Review → Export: set up the clip, pick music, style captions, then bake and download. Accounts and Premium live on the web at [pyclips.in](https://pyclips.in) — sign in with Google, then use the same account in the Windows app.

**Free** includes a limited number of clips. **Premium** unlocks unlimited bakes and the full caption library. India is billed in INR (₹29 / month or ₹199 / year). Elsewhere is billed in USD ($2.99 / month or $29.99 / year) through Razorpay.

- Website: [pyclips.in](https://pyclips.in)
- X: [x.com/Py_Clips](https://x.com/Py_Clips)


